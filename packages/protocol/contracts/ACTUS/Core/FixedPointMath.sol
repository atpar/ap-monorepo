...
library FixedPointMath {
     ...
     */
    function fixedMul(int256 a, int256 b)
        internal
        pure
        returns (int256)
    {
        ...
        require(!(a == -1 && b == INT256_MIN), "FixedPointMath.fixedMul: OVERFLOW_DETECTED");
        int256 c = a * b;
        require(c / a == b, "FixedPointMath.fixedMul: OVERFLOW_DETECTED");

        // normalize (divide by MULTIPLICATOR)
        int256 d = c / int256(MULTIPLICATOR);
        require(d != 0, "FixedPointMath.fixedMul: CANNOT_REPRESENT_GRANULARITY");

        return d;
    }

    function fixedDiv(int256 a, int256 b)
        internal
        pure
        returns (int256)
    {
        require(b != 0, "FixedPointMath.fixedDiv: DIVIDED_BY_ZERO");

        // normalize (multiply by MULTIPLICATOR)
        if (a == 0) return 0;
        int256 c = a * int256(MULTIPLICATOR);
        require(c / a == int256(MULTIPLICATOR), "FixedPointMath.fixedDiv: OVERFLOW_DETECTED");

        require(!(b == -1 && a == INT256_MIN), "FixedPointMath.fixedDiv: OVERFLOW_DETECTED");
        int256 d = c / b;
