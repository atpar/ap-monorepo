...
describe('SimpleRestrictedCMTA20FDT', () => {
  ...
  const snapshotTaker = (self) => getSnapshotTaker(buidlerRuntime, self, async () => {
    ...
    await self.simpleRestrictedRuleEngine.methods.updateOutboundWhitelistEnabled('1', '1', true).send({from: owner});
    await self.simpleRestrictedRuleEngine.methods.addToWhitelist(tokenHolder1, '1').send({from: owner});
    await self.simpleRestrictedRuleEngine.methods.addToWhitelist(tokenHolder2, '1').send({from: owner});
    await self.simpleRestrictedRuleEngine.methods.addToWhitelist(tokenHolder3, '1').send({from: owner});
    await self.simpleRestrictedRuleEngine.methods.addToWhitelist(anyone, '1').send({from: owner});
  });
