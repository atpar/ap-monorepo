...
describe('COLLAEngine', () => {
  ...
  before(async () => {
    ...
    this.terms = await getDefaultTestTerms('COLLA');
  });

  const computeEventScheduleSegment = async (terms, segmentStart, segmentEnd) => {
    const schedule = [];

    schedule.push(... await this.COLLAEngineInstance.methods.computeNonCyclicScheduleSegment(
      terms,
      segmentStart,
      segmentEnd
    ).call());
    schedule.push(... await this.COLLAEngineInstance.methods.computeCyclicScheduleSegment(
      terms,
      segmentStart,
      segmentEnd,
      eventIndex('IPCI')
  ...
  it('should yield the initial contract state', async () => {
    const initialState = await this.COLLAEngineInstance.methods.computeInitialState(this.terms).call();
  ...
  it('should yield the next next contract state and the contract events', async () => {
    const initialState = await this.COLLAEngineInstance.methods.computeInitialState(this.terms).call();
    const schedule = await this.COLLAEngineInstance.methods.computeNonCyclicScheduleSegment(
      this.terms,
      0,
      this.terms.maturityDate
    ).call();
    const nextState = await this.COLLAEngineInstance.methods.computeStateForEvent(
      this.terms,
      initialState,
      schedule[0],
