...
describe('COLLAActor', () => {
  ...
  const snapshotTaker = (self) => getSnapshotTaker(buidlerRuntime, self, async () => {
    ...
    self.terms = {
      ...
      collateralCurrency: paymentTokenAddress,
    };
  ...
  it('should initialize an asset', async () => {
    const storedTerms = await this.COLLARegistryInstance.methods
    ...
    const storedOwnership = await this.COLLARegistryInstance.methods
      .getOwnership(web3.utils.toHex(this.assetId)).call();
    const storedEngineAddress = await this.COLLARegistryInstance.methods
  ...
  it('should process next state with contract status equal to PF', async () => {
    const _event = await this.COLLARegistryInstance.methods
    ...
    const { events } = await this.COLLAActorInstance.methods.progress(web3.utils.toHex(this.assetId))
      .send({ from: creatorObligor });
    ...
    const isEventSettled = await this.COLLARegistryInstance.methods
  ...
  it('should process next state transitioning from PF to DL', async () => {
    ...
    const storedFinalizedState = web3ResponseToState(
      await this.COLLARegistryInstance.methods.getFinalizedState(web3.utils.toHex(this.assetId)).call()
    );
    const isEventSettled = await this.COLLARegistryInstance.methods
      .isEventSettled(web3.utils.toHex(this.assetId), _event).call();
  ...
  it('should process next state transitioning from DL to PF', async () => {
    ...
    const storedPendingEvent_DL = await this.COLLARegistryInstance.methods
      .getPendingEvent(web3.utils.toHex(this.assetId)).call();
    const isEventSettled_DL = await this.COLLARegistryInstance.methods
      .isEventSettled(web3.utils.toHex(this.assetId), _event).call();
    ...
    const { events: events_PF } = await this.COLLAActorInstance.methods.progress(web3.utils.toHex(this.assetId))
      .send({ from: nobody });
    ...
    const storedPendingEvent_PF = await this.COLLARegistryInstance.methods
      .getPendingEvent(web3.utils.toHex(this.assetId)).call();
    const isEventSettled_PF = await this.COLLARegistryInstance.methods
      .isEventSettled(web3.utils.toHex(this.assetId), _event).call();
