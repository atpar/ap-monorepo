...
library CEGEncoder {
    ...
    function encodeAndSetCEGTerms(Asset storage asset, CEGTerms memory terms) internal {
        ...
        storeInPackedTerms(
            asset,
            "contractReference_1_object", //TODO ??
        ...
        storeInPackedTerms(
            asset,
            "contractReference_2_object", //TODO ??
    ...
    function decodeAndGetCEGTerms(Asset storage asset) internal view returns (CEGTerms memory) {
        return CEGTerms(
            ...
            ContractReference(
                asset.packedTerms["contractReference_1_object"], //TODO ??
                ContractReferenceType(uint8(uint256(asset.packedTerms["contractReference_1"] >> 16))),
                ContractReferenceRole(uint8(uint256(asset.packedTerms["contractReference_1"] >> 8))),
            )
            ContractReference(
                asset.packedTerms["contractReference_2_object"], //TODO ??
                ContractReferenceType(uint8(uint256(asset.packedTerms["contractReference_2"] >> 16))),
                ContractReferenceRole(uint8(uint256(asset.packedTerms["contractReference_2"] >> 8))),
            )
        );
    }
