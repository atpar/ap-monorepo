...
const PAMStatefulContractOnChain_ExplicitArtifact = artifacts.require('PAMStatefulContractOnChain_Explicit.sol')
const PAMStatefulContractOnChain_Explicit = require('../build/contracts/PAMStatefulContractOnChain_Explicit.json')
...
contract('PAMStatefulContractOnChain_Explicit', (accounts) => {

  let PAMStatefulContractOnChain_ExplicitDeployed
  let contractTerms
  let precision
  ...
  before(async () => {
    ...
    const PAMStatefulContractOnChain_ExplicitInstance = await PAMStatefulContractOnChain_ExplicitArtifact.new(
    ...
    PAMStatefulContractOnChain_ExplicitDeployed = new web3.eth.Contract(PAMStatefulContractOnChain_Explicit.abi, PAMStatefulContractOnChain_ExplicitInstance.address);
  ...
  it('should yield the first contract state and the event schedule', async () => {
    await PAMStatefulContractOnChain_ExplicitDeployed.methods.initialize(contractTerms, 86400, 7*86400)
    .send({from: accounts[0], gas: 2000000})
    // .on('receipt', (receipt) => { console.log(receipt) })
    const contractEventSchedule = await PAMStatefulContractOnChain_ExplicitDeployed.methods.getContractEventSchedule().call()
  ...
  it('should process principal payment, emit contract event and yield the next contract state ', async () => {
    ...
    await PAMStatefulContractOnChain_ExplicitDeployed.methods.payPrincipal(contractTerms.initialExchangeDate)
  ...
  it('should process all interest payments, emit the contract events and yield the resulting contract state ', async () => {
    ...
    let contractState = await PAMStatefulContractOnChain_ExplicitDeployed.methods.getContractState().call()
    let contractEventScheduleIndex = await PAMStatefulContractOnChain_ExplicitDeployed.methods.getContractEventScheduleIndex().call()
    ...
    while(contractEventSchedule[contractEventScheduleIndex][0] === '4') {
      ...
      await PAMStatefulContractOnChain_ExplicitDeployed.methods.payInterest(contractEvent.scheduledTime)
  ...
  it('should redeem principal, emit contract event and yield the next contract state ', async () => {
    ...
    await PAMStatefulContractOnChain_ExplicitDeployed.methods.redeemPrinicipal(contractTerms.maturityDate)
