...
contract('ClaimsTokenERC20', (accounts) => {
  ...
  beforeEach(async () => {
    this.SampleTokenInstance = await SampleToken.new()
    this.ClaimsTokenERC20Instance = await ClaimsTokenERC20.new(ownerA, this.SampleTokenInstance.address)

    this.totalSupply = await this.ClaimsTokenERC20Instance.SUPPLY() 
    ...
    // first deposit
    await this.SampleTokenInstance.transfer(this.ClaimsTokenERC20Instance.address, web3.utils.toHex(depositAmount))
  })

  it('should increment <totalReceivedFunds> after deposit', async () => {
    const claimsTokenERC20Balance = await this.SampleTokenInstance.balanceOf(this.ClaimsTokenERC20Instance.address)
  ...
  it('should withdraw <newFundsReceived> amount for user', async () => {
    const totalReceivedFunds = await this.ClaimsTokenERC20Instance.totalReceivedFunds()
    const preClaimsTokenERC20Balance = await this.SampleTokenInstance.balanceOf(this.ClaimsTokenERC20Instance.address)

    await this.ClaimsTokenERC20Instance.withdrawFunds({ from: ownerD })

    const postClaimsTokenERC20Balance = await this.SampleTokenInstance.balanceOf(this.ClaimsTokenERC20Instance.address)
