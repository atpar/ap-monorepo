...
import { ANNEngine } from '../wrappers/ANNEngine';
import { ContractTerms, ContractState, ContractEvent, EvaluatedEventSchedule, ProtoEventSchedule } from '../types';


/**
 * contains economic logic for a ANN contract
 */
export class ANN implements ContractEngine {

  protected annEngine: ANNEngine;

  public constructor (annEngine: ANNEngine) {
  ...
  public async computeInitialState (terms: ContractTerms): Promise<ContractState> {
    const initialState = await this.annEngine.computeInitialState(
      terms
    ).call();
    return initialState;
  }
  ...
  public async computeNextState (
    ...
    const { nextState } = await this.annEngine.computeNextState(
      terms, 
      state, 
      timestamp
    ).call();
  ...
  public async validateInitialState (
    ...
    const initialState = await this.annEngine.computeInitialState(terms).call();
  ...
  public async validateNextState (
    ...
    const { nextState: actualState } = await this.annEngine.computeNextState(
      terms, 
      state,
  ...
  public async computeInitialProtoEventSchedule (terms: ContractTerms): Promise<ProtoEventSchedule> {
    const protoEventSchedule: ProtoEventSchedule = await this.annEngine.computeProtoEventScheduleSegment(
      terms,
      terms.statusDate,
      terms.maturityDate
    ).call();
  ...
  public async computePendingProtoEventSchedule (
    ...
    const protoEventSchedule = await this.annEngine.computeProtoEventScheduleSegment(
      terms, 
  ...
  public async computeEvaluatedInitialSchedule (terms: ContractTerms): Promise<EvaluatedEventSchedule> {
    const initialContractState: ContractState = await this.annEngine.computeInitialState(terms).call();
    const protoEventSchedule: ProtoEventSchedule = await this.annEngine.computeProtoEventScheduleSegment(
      terms,
      terms.statusDate,
      terms.maturityDate
    ...
    for (let protoEvent of protoEventSchedule) {
      const response = await this.annEngine.computeNextStateForProtoEvent(
        terms,
        state,
        protoEvent,
